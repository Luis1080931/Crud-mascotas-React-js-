...
const ActualizarMascota = () => {
    ...
    useEffect(() => {
        if (mascota) {
            console.log('Mascoya:', mascota);
            setFormData({
                nombre: mascota[0].nombre_mascota,
                raza: mascota[0].raza,
                categoria: mascota[0].categoria,
                image: mascota[0].image,
                genero: mascota[0].genero
            });
            setRaza(mascota[0].raza)
    ...
    const handleSubmit = (e) => {
        ...
        try {
            axiosClient.put(`/mascotas/actualizar/${id}`, datosSubmit).then((response) => {
    ...
    return (
        ...
        >
            ...
            <div className='mt-16'>
                <img className='rounded-full' src={`http://localhost:3000/img/${mascota.image}`} alt="" />
            </div>
